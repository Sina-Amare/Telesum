...
async def main(phone):
    ...
    print(f"Logged in as: {user.first_name} ({user.phone})")
            ...
            if chat_id:
                ...
                if filter_type:
                    ...
                    if messages:
                        ...
                        print("\nMessages:")
                        for i, (sender, msg, timestamp) in enumerate(messages, 1):
                            print(
                ...
                if chat_id:
                    ...
                    if filter_type:
                        ...
                        if messages:
                            print("\nMessages:")
                            for i, (sender, msg, timestamp) in enumerate(messages, 1):
                                print(f"{i W}. {sender}: {msg} ({timestamp.strftime('%Y-%m-%d %H:%M:%S')})")
                        else:
                            if filter_type == "recent_messages":
                                print(
                                    f"\nNo messages found in the last {filter_value} messages!")
...
async def get_message_filter(telegram):
    ...
    if choice == "1":
        ...
        while True:
            ...
            # اگه تاریخ نامعتبر بود، دوباره می‌پرسه
